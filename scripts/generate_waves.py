...
# Reuse the SO line flattener so the audit parquet matches the existing
# raw schema and downstream code keeps working. ``scripts/`` isn't a
# package; load by file path.
import importlib.util  # noqa: E402

_EXTRACT_PATH = Path(__file__).resolve().parent / "extract.py"
_spec = importlib.util.spec_from_file_location("_extract_mod", _EXTRACT_PATH)
_extract_mod = importlib.util.module_from_spec(_spec)  # type: ignore[arg-type]
_spec.loader.exec_module(_extract_mod)  # type: ignore[union-attr]
_flatten_outbound_order_lines = _extract_mod._flatten_outbound_order_lines
    ...
    *,
    ...
    try:
        for order in search_outbound_orders(
            ...
            n_orders += 1
            rows.extend(_flatten_outbound_order_lines(order))
